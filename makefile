...
VERSION = -std=c++17 #derniere version de c++
...
HEAD=$(filter-out main.h , $(SRC:.cpp=.h)) #converti tout les .cpp en .h sauf main.cpp...
DEBUG=no
WARNING=no
MEMCHECK= #-g
...
$(EXEC) : $(OBJ) # donc on appelle ainsi EXEC qui regarde si la liste de .o a besoin d etre recompile
	$(CC) $(OBJ) $(VERSION) -L$(PATH_lib) $(LIBS) $(INC) -o $(EXEC) $(OPT) $(FLAG) $(MEMCHECK) $(WARN)
...
.cpp.o: #ca definit la regle generique a utiliser si on a besoin de creer un .o tout en verifiant que le .cpp a bien ete change depuis la derniere modification du .o
	$(CC) $(VERSION) -L$(PATH_lib) $(LIBS) -I$(INC) $(OPT) -c $< -o $@ $(FLAG) $(WARN)
